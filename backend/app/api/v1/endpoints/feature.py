...
def create_feature(
    *,
    ...
    """
    # 1. Verificar que el Feature Model existe y que el usuario tiene permisos
    feature_model = crud.get_feature_model(
        session=session, feature_model_id=feature_in.feature_model_id
    )
    ...
    # 2. (Opcional pero recomendado) Verificar que el parent_id, si existe, pertenece al mismo modelo
    if feature_in.parent_id:
        ...
        if (
            not parent_feature
            or parent_feature.feature_model_id != feature_in.feature_model_id
        ):
            raise HTTPException(
                status_code=400,
    ...
    try:
        feature = crud.create_feature(session=session, feature_create=feature_in)
        return feature
...
def update_feature(
    *,
    ...
    if (
        db_feature.feature_model.owner_id != current_user.id
    ...
    try:
        updated_feature = crud.update_feature(
            session=session, db_feature=db_feature, feature_in=feature_in
        )
...
def delete_feature(
    *,
    ...
    crud.delete_feature(session=session, db_feature=db_feature)
