...
def test_get_users_normal_user_me(
    ...
    r = client.get(
        f"{settings.API_V1_PREFIX}/users/me", headers=normal_user_token_headers
    )
    current_user = r.json()
    assert current_user
    assert current_user["is_active"] is True
...
def test_get_existing_user(
    ...
    assert 200 <= r.status_code < 300
    api_user = r.json()
    existing_user = user_repo.get_by_email(email=username)
...
def test_update_user(
    ...
    """Test actualizar email de un usuario por superusuario."""
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user_repo = UserRepositorySync(db)
...
def test_update_user_not_exists(
    ...
    """Test actualizar usuario que no existe devuelve 404."""
...
def test_update_user_email_exists(
    ...
    password2 = random_lower_string()
    user_in2 = UserCreate(email=username2, password=password2)
    user2 = user_repo.create(user_in2)
...
def test_activate_user(
    ...
    """Test activar un usuario desactivado."""
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user_repo = UserRepositorySync(db)
    ...
    # Activar vía API
    r = client.patch(
        f"{settings.API_V1_PREFIX}/users/{user.id}/activate/",
        headers=superuser_token_headers,
    )
    assert r.status_code == 200
    activated_user = r.json()
...
def test_activate_user_not_found(
    ...
    """Test activar usuario que no existe devuelve 404."""
    r = client.patch(
        f"{settings.API_V1_PREFIX}/users/{uuid.uuid4()}/activate/",
        headers=superuser_token_headers,
    )
    assert r.status_code == 404
    assert r.json()["detail"] == "User not found"
...
def test_activate_user_without_privileges(
    client: TestClient, normal_user_token_headers: dict[str, str], db: Session
) -> None:
    """Test usuario normal no puede activar usuarios."""
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user_repo = UserRepositorySync(db)
...
def test_deactivate_user(
    ...
    """Test desactivar un usuario activo."""
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user_repo = UserRepositorySync(db)
    ...
    # Desactivar vía API
    r = client.patch(
        f"{settings.API_V1_PREFIX}/users/{user.id}/deactivate/",
        headers=superuser_token_headers,
    )
    assert r.status_code == 200
    deactivated_user = r.json()
...
def test_deactivate_user_not_found(
    ...
    """Test desactivar usuario que no existe devuelve 404."""
    r = client.patch(
        f"{settings.API_V1_PREFIX}/users/{uuid.uuid4()}/deactivate/",
        headers=superuser_token_headers,
    )
    assert r.status_code == 404
    assert r.json()["detail"] == "User not found"
...
def test_deactivate_user_without_privileges(
    client: TestClient, normal_user_token_headers: dict[str, str], db: Session
) -> None:
    """Test usuario normal no puede desactivar usuarios."""
    username = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    user_repo = UserRepositorySync(db)
...
def test_deactivate_yourself_error(
    client: TestClient, superuser_token_headers: dict[str, str], db: Session
) -> None:
    ...
    r = client.patch(
        f"{settings.API_V1_PREFIX}/users/{super_user.id}/deactivate/",
        headers=superuser_token_headers,
    )
    assert r.status_code == 403
